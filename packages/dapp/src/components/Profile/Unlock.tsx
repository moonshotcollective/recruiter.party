...
function UnlockProfile({ privateProfile }) {
  ...
  useEffect(() => {
    async function exec() {
      ...
      const allowance = await tokenContract.allowance(account, dRecruitContract.address);
        ...
        <ModalContent>
          ...
          <ModalBody>
            <VStack justifyItems="left">
              <FormLabel htmlFor="stakeAmount">Enter stake amount in {tokenSymbol}</FormLabel>
              <Input
                value={stakeAmount}
